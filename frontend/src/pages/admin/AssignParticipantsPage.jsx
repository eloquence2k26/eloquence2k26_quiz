...
export default function AssignParticipantsPage() {
  ...
  const [selectedQuizId, setSelectedQuizId] = useState(location.state?.quizId || '');
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (qRes.success && qRes.data) {
        ...
        const preferredId = location.state?.quizId || (qRes.data.length > 0 ? qRes.data[0].id : '');
        if (preferredId) {
          setSelectedQuizId(preferredId);
        }
      }
